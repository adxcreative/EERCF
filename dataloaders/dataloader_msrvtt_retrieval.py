...
class MSRVTT_TrainDataLoader_Multi(Dataset):
    """MSRVTT train dataset loader."""
    def __init__(
            self,
            csv_path,
        ...
        self.csv = pd.read_csv(csv_path)
        json_path_list = json_path.strip().split("+")
        print("multi json length:{}".format(len(json_path_list)))
        self.data_list = [json.load(open(x, 'r')) for x in json_path_list]
        print("multi json load over.")
        # self.data = json.load(open(json_path, 'r'))
        self.features_path = features_path
        self.feature_framerate = feature_framerate
        self.max_words = max_words
        ...
        if self.unfold_sentences:
            train_video_ids = list(self.csv['video_id'].values)
            self.sentences_dict_list = []
            for data in self.data_list:
                sentences_dict = {}
                for itm in data['sentences']:
                    if itm['video_id'] in train_video_ids:
                        sentences_dict[len(sentences_dict)] = (itm['video_id'], itm['caption'])
                self.sentences_dict_list.append(sentences_dict)
            self.sample_len = len(self.sentences_dict_list[0])
        
        self.rawVideoExtractor = RawVideoExtractor(framerate=feature_framerate, size=image_resolution)
        self.SPECIAL_TOKEN = {"CLS_TOKEN": "<|startoftext|>", "SEP_TOKEN": "<|endoftext|>",
                              "MASK_TOKEN": "[MASK]", "UNK_TOKEN": "[UNK]", "PAD_TOKEN": "[PAD]"}
        print("dataset init over")
    ...
    def __getitem__(self, idx):
        ...
        multi_pairs_mask = []
        multi_pairs_segment = []
        for sentences_dict in self.sentences_dict_list:
            if self.unfold_sentences:
                video_id, caption = sentences_dict[idx]
            pairs_text, pairs_mask, pairs_segment, choice_video_ids = self._get_text(video_id, caption)
            # print(choice_video_ids)
            multi_pairs_text.append(pairs_text)
            multi_pairs_mask.append(pairs_mask)
            multi_pairs_segment.append(pairs_segment)
        multi_pairs_text = np.stack(multi_pairs_text,0)
        multi_pairs_mask = np.stack(multi_pairs_mask,0)
        multi_pairs_segment = np.stack(multi_pairs_segment,0)

        video, video_mask = self._get_rawvideo(choice_video_ids)
        return multi_pairs_text, multi_pairs_mask, multi_pairs_segment, video, video_mask



class MSRVTT_TrainDataLoader_Multi_Negative(Dataset):
    """MSRVTT train dataset loader."""
    def __init__(
            self,
            csv_path,
            ...
            frame_order=0,
            slice_framepos=0,
            maxsim_file = "/data/tiankaibin/xclipfine/ckpts_msrvtt/xclip_msrvtt_vit32_parematrix2/max20sim.npy"
    ):
        self.maxsim20 = np.load(maxsim_file)
        print("maxsim.shape",self.maxsim20)
        self.csv = pd.read_csv(csv_path)
        json_path_list = json_path.strip().split("+")
        print("multi json length:{}".format(len(json_path_list)))
        self.data_list = [json.load(open(x, 'r')) for x in json_path_list]
    ...
    def __getitem__(self, idx):
        ...
        #get simidxs
        repeat_num = 20
        simidxs = self.maxsim20[idx]
        simidxs = list(simidxs)
        if int(idx//repeat_num) in simidxs:
            simidxs.remove(idx//repeat_num)
        realsimidxs = np.random.choice(simidxs, 1, replace=False)
        realsimidxs = [x*repeat_num+int(np.random.randint(repeat_num)) for x in realsimidxs]

        simidxs = [idx] + realsimidxs
        ...
        for idx in simidxs:
            video_id, caption = self.sentences_dict_list[0][idx]
            pairs_text, pairs_mask, pairs_segment, choice_video_ids = self._get_text(video_id, caption)
            video, video_mask = self._get_rawvideo(choice_video_ids)
            mm_pairs_text.append(pairs_text)
            mm_pairs_mask.append(pairs_mask)
            mm_pairs_segment.append(pairs_segment)
            mm_pairs_video.append(video)
            mm_pairs_video_mask.append(video_mask)
        
        multi_pairs_text = np.stack(mm_pairs_text,0)
        multi_pairs_mask = np.stack(mm_pairs_mask,0)
        multi_pairs_segment = np.stack(mm_pairs_segment,0)
        multi_pairs_video = np.stack(mm_pairs_video,0)
        multi_pairs_video_mask = np.stack(mm_pairs_video_mask,0)

        

        return multi_pairs_text, multi_pairs_mask, multi_pairs_segment, multi_pairs_video, multi_pairs_video_mask


class MSRVTT_TrainDataLoader_Caption(Dataset):
    """MSRVTT train dataset loader."""
    def __init__(
            self,
            csv_path,
